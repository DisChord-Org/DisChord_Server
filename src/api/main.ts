...
import fs from 'fs';
import runtime from 'src/runtime-instance';

const app = express();
const PORT = runtime === 'dev'? 3000 : 45350;
const ASSETS_PATH = runtime === 'dev'? '/releases-test/releases' : '/var/www/dischord-assets/releases';
...
app.get('/version/:component', (req, res) => {
    ...
    const releasesPath: string = path.join(process.cwd(), ASSETS_PATH, component, 'version.json');
...
app.get('/download/:component/:version', (req, res) => {
    ...
    const releasePath: string = path.join(process.cwd(), ASSETS_PATH, component, `${component}-v${version}.exe`);
